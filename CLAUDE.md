...
- **Spell tricky words/brands phonetically so TTS says them right:** write it the way it actually SOUNDS, not just letter-by-letter. Some are letters ("p-p-a-r-g", "f-t-o"), some are words/syllables ("In and Out"; CYP1A2 = "sip-one-ay-two"). Never trust the model to pronounce acronyms or brand names. For any non-obvious gene, CONFIRM the pronunciation with the user before generating, then record it in GENE_TALLY.md so it's reused (never re-guessed).
